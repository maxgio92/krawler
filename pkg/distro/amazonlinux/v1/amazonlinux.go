...
func dereferenceRepositoryURL(src *url.URL, arch distro.Arch) (*url.URL, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("Amazon Linux v1 repository URL not valid to be dereferenced")
	}

	if resp.Body == nil {
		return nil, fmt.Errorf("empty response from Amazon Linux v1 repository reference URL")
	}
